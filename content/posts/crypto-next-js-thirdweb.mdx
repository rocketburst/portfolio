...
Go to the `pages/_app.js` file and add the following code:
...
Now let's use the `useWeb3` hook from thirdweb to connect our wallet. We will extract the `address` property and the `connectWallet` function.
...
```jsx
<a
  className="inline-block p-[2px] rounded bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 hover:text-white active:text-opacity-75 focus:outline-none focus:ring cursor-pointer"
